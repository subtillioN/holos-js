 ...
 * @param $loop  {Boolean }
 */
SZ.Cursor = function ($last, $first, $loop) {
	var _first = 0;
	var _last = 9;
	var _loop = true;
	var _i = 0;
	...
	function _setBounds($first, $last) {
		if($last < $first) {
			SZ.log(this + " ERROR :: BOUNDS, LAST (" + $last + ") SHOULD BE GREATER THAN FIRST (" + $first + ").");
			return;
		}
		_first = $first;
		_last = $last;
	...
	this.getLast = function () { return _last;};

	this.hasPrev = function () {return (_i - 1 >= _first);};

	this.hasNext = function () { return (_i + 1 <= _last);};

	this.getLength = function () {return _length;};

	this.i = function() {return _i;};

	this.setUpdateHandler = function (value) {_updateHandler = value;};
	...
	this.doesLoop = function (loop) {_loop = loop;};
